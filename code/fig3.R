...
pan_a <- ggplot(df, aes(x = x)) +
  ...
  annotate("text", x = -1.75, y = -2.75, label = "Carbon", color = "#00B050", size = 4, hjust = 0) +
  annotate("text", x = -1.75, y = -3.25, label = "sink", color = "#00B050", size = 4, hjust = 0) +
  ...
  annotate("text", x = 0, y = -0.8, label = expression(italic(E[P]) == 0.32~eV), color = "#00B050", size = 4) +
  annotate("text", x = 8.75, y = 3.75, label = expression(italic(E[R]) == 0.65~eV), color = "#ED7D31", size = 4) +
  xlab(expression(paste('Reciprocal thermal energy, ', '-1/',italic('k')[italic("B")], italic('T'), ' (',  eV^{-1}, ')'))) +
  ylab(expression(paste("ln(Photosynthesis or respiration)"))) +
...
pan_b <- ggplot(df, aes(x = x)) +
  ...
  annotate("text", x = 5, y = 0.6, label = "Carbon", color = "#00B050", size = 4, hjust = 0) +
  annotate("text", x = 5, y = 0.1, label = "sink", color = "#00B050", size = 4, hjust = 0) +
  annotate("text", x = 0, y = -0.3, label = expression(italic(E[P]) == 0.65~eV), color = "#00B050", size = 4) +
  annotate("text", x = 9.75, y = -0.1, label = expression(italic(E[R]) == 0.65~eV), color = "#ED7D31", size = 4) +
  xlab(expression(paste('Reciprocal thermal energy, ', '-1/',italic('k')[italic("B")], italic('T'), ' (',  eV^{-1}, ')'))) +
  ylab(expression(paste("ln(Photosynthesis or respiration)"))) +
  scale_x_continuous(sec.axis = sec_axis(trans = ~ .*1 , name = expression(paste('Temperature'~(degree*C))))) +
  theme_bw(base_size=12) +
...
fig3 <- ggarrange(pan_a, pan_b, common.legend = F, legend = "right", labels = "auto")
fig3
